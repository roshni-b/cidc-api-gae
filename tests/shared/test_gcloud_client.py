...
def test_grant_download_access(monkeypatch):
    ...
    matching_binding = {
        "condition": {
            "title": "Conditional download access for test@email.com",
            "description": "Auto-generated by the CIDC API on 2020-08-27 22:39:46.946004",
            "expression": _build_download_expression(
                [
    ...
    def set_iam_policy(policy):
        bindings = policy.bindings
        assert len(bindings) == 2
        expression = bindings[1]["condition"]["expression"]
        assert len(expression.split(" || ")) == 2
        assert "10021/wes" in expression
        assert "10021/participants" in expression
...
def test_revoke_download_access(monkeypatch):
    ...
    other_binding = {"role": "some-other-role", "members": {f"user:JohnDoe"}}

    def set_iam_policy(policy):
        assert len(policy.bindings) == 2
        assert "10021/wes" not in policy.bindings[1]["condition"]["expression"]
